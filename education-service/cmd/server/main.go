...
func main() {
	...
	// Connect to read replica (fallback to primary if not available)
	replicaDB, err := database.ConnectPostgresReplica()
	if err != nil {
		log.Printf("Failed to connect to replica database: %v", err)
		log.Println("Using primary database for read operations")
		replicaDB = db
	} else {
